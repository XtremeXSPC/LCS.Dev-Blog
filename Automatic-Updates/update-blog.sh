...
# ======================================================= #
# Logging Setup
# Set up logging at the beginning to capture all logs,
...
# Trap enanched to catch interrupts
...
# Add default values for variables if not set
...
sync_posts() {
    ...
    # Creation of a backup directory with the current date and time
...
generate_file_hashes() {
    ...
    # If the hash file exists, create a backup before generating new hashes
    if [ -f "$hash_file" ]; then
    ...
    # Verify that the hash file was generated successfully
    if [ ! -f "$hash_file" ]; then
...
build_hugo_site() {
    ...
    if ! hugo; then
        cd "$current_dir" || true  # Return to the original directory in case of an error
...
push_to_main() {
    ...
    # Verify connection to the remote repository before attempting to push
    if ! git ls-remote --exit-code origin &>/dev/null; then
...
# Load file hashes before starting the process
load_file_hashes
